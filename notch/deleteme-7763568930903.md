...
#### Actions to perform :
  ...
  - create file       : configs/environments/${env}/${flashblade_account}/ » et créer un fichier ${client_name}.yaml 
  - using format      : 
```
      environment: ${env}-gcp
      replication: true
...
### 2.1 : Fill in the global RabbitMQ configuration for the satellites to be deployed.
        ...
        - applicationName: gisaio
          ...
          exchanges:
            - exchangeVersion: v1
              ...
              arguments:
                - "x-match: all"
                - "alternate-exchange: ${env}-gcp-gisaio-router-exchange-alternate"
              queues:
                - name: "entities"
                  version: v1
                  arguments: 
                    - "x-dead-letter-exchange: ${env}-gcp-gisaio-entities-exchange-dlx"
            - exchangeVersion: dlx
              type: fanout
              vhost: ${env}-infrastructure
              queueRef: entities
            ...
            - exchangeVersion: v1
              ...
              queues:
                - name: "invoice"
                  version: v1
                  arguments: 
                    - "x-dead-letter-exchange: ${env}-gcp-gisaio-invoice-exchange-dlx"
                  bindings:
                    - version: v1
                      arguments:
                        - arg:
                ...
                - name: "process"
                  version: v1
                  arguments: 
                    - "x-dead-letter-exchange: ${env}-gcp-gisaio-process-exchange-dlx"
                  bindings:
                    - version: v1
                      arguments:
                        - arg:
...
### 2.2 : Fill in the configuration for each satellite to be deployed
...
```
          ...
          preferredDuringSchedulingIgnoredDuringExecution:
            - weight: 100
              podAffinityTerm:
                labelSelector:
                  matchLabels:
                    app: gis${satellite_short_name}-${env}-gcp
      ...
      env:
        ...
        BROKER_EXCHANGE: "${env}-gcp-gisaio-router-exchange-v1"
        RABBITMQ_PORT: 5672
      ...
      readinessProbe:
        httpGet:
          path: /gcp/invoice-processing/actuator/health/readiness
          port: 8080
          scheme: HTTP
      ...
      livenessProbe:
        httpGet:
          path: /gcp/invoice-processing/actuator/health/liveness
          port: 8080
          scheme: HTTP
      ...
      rabbitmq:
        ...
        user:
          ...
          permissions:
            - vhost: ${env}-invoicing
              write: '^${env}-gcp-gisaio-router-exchange-v([1-9][0-9]?)$'
          extras:
            - keyUsername: RABBITMQ_USERNAME_GISAIO
              ...
              permissions:
                - vhost: ${env}-invoicing
                  write: '^${env}-gcp-gisaio-router-exchange-v([1-9][0-9]?)$'
                  configure: "^$"
                  read: '^(${env}-gcp-gisaio-process-queue-v([1-9][0-9]?)|${env}-gcp-gisaio-invoice-queue-v([1-9][0-9]?))$'
                - vhost: ${env}-infrastructure
                  write: '^${env}-gcp-gisaio-router-exchange-v([1-9][0-9]?)$'
                  configure: "^$" 
                  read: '^${env}-gcp-gisaio-entities-queue-v([1-9][0-9]?)$'
...
#### INFO : Create the secrets for each entry under ```secrets:``` in every values.yaml file that was created or modified
  ...
  - S3:FlashBlade secrets were created by the GitHub Action “Flashblade provisioning” and values are available
    ...
    - at path  : Secrets / generix / flashblade / uat / acc-ocp-uat-gnx / gcp / uat-invoiceapi-gcp
    - keys     : 
      - access_key_id
      - secret_access_key  
    - Ex : secrets:
...
#### Actions to perform:
  ...
  - Create the RabbitMQ access secrets for the other applications (Ex: AIO) :
    ...
    - "Secret data" key   : ${secret_name}  ; Ex pour RABBITMQ_USERNAME : au format "${env}-${client_name}-gisaio"
...
```
  ...
  - at path.      : vault/apps-registry/kustomization.yaml
    - add line : 
```
        ${client_name}-${satellite_name}-${env}.yaml=generix/gis/${client_name}/${satellite_name}/${env}.yaml
```
    - commit the file to git repo 
...
### Middlewares web interfaces : 
  ...
  - PRD:Vault     : https://vault-console.apps.prd.openshift.vmwr/
  - NPD:Vault     : https://vault-console.apps.npr.openshift.vmwr/
